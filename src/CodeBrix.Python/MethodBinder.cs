...
using System.Text;
using MaybeMethodBase = CodeBrix.Python.MaybeMethodBase<System.Reflection.MethodBase>;
    ...
    private static Binding? Bind(BorrowedReference inst, BorrowedReference args, Dictionary<string, PyObject> kwargDict, MethodBase[] methods, bool matchGenerics, bool argsReversed = false)
    {
        ...
        if (argMatchedMethods.Count > 0)
        {
            ...
            if (!mi.IsStatic && inst != null)
            {
                ...
                // 'CodeBrix.Python.ClassObject' to type 'CodeBrix.Python.CLRObject'
    ...
    static BorrowedReference HandleParamsArray(BorrowedReference args, int arrayStart, int pyArgCount, out NewReference tempObject)
    {
        ...
        if ((pyArgCount - arrayStart) == 1)
        {
            ...
            else
            {
                ...
                op = tempObject.IsNull() ? throw new NullReferenceException() : new BorrowedReference(NewReference.DangerousGetAddressOrNull(tempObject)); // CodeBrix port: inlined Borrow() to decouple from the out NewReference for net10 ref-safety
            }
        }
        else
        {
