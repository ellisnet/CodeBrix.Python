...
def test_simple_callback_to_python():
    ...
    from CodeBrix.Python.TestSupport import ThreadTest
...
def test_concurrent_clr_object_creation():
    ...
    crash that is also reproducible in the upstream project, and is outside
...
def test_concurrent_delegate_creation():
    ...
    """
    from CodeBrix.Python import PythonEngine
...
def test_concurrent_clr_delegate_invocation_from_python():
    ...
    Real-world: embedders pass Python callables where C# expects a delegate;
    under FT the dispatcher emit + Invoke run from multiple threads.  Hits DelegateManager.GetDispatcher (Reflection.Emit lock)
    and Dispatcher.Dispatch (Py.GIL reacquisition).
    """
    from CodeBrix.Python.TestSupport import (
...
def test_concurrent_generic_type_binding():
    ...
    Real-world: the upstream project's issues #2269, #1407 and #821 -
    concurrent ToPython / GenericByName from N threads.  Exercises ClassManager.cache,
...
def test_concurrent_gc_collect_on_clr_cycles():
    ...
    def churn(_):
        ...
        # when the .NET GC happens to run during construction.  Across a range
        # of interpreters this catches regressions reliably without the
        # heavyweight CLR GC.Collect that deadlocks Mono on x64 Linux.
        for _ in range(100):
            a, b = Cycle(), Cycle()
            a.peer = b
            b.peer = a
