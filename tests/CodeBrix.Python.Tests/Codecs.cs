...
public class Codecs : IDisposable
{
    // Reset the process-wide codec registry both before and after each test so
    // these globally-stateful tests are isolated regardless of run order.
    public Codecs()
    {
    ...
    {
        TupleConversionsGenericImpl<ValueTuple<int, string, object>, ValueTuple>();
    }

    static void TupleConversionsGenericImpl<T, TTuple>()
    {
        ...
        using (var scope = Py.CreateScope())
        {
            ...
            Assert.Equal(expected: tuple, actual: restored);
        }
    }
    ...
    {
        TupleConversionsObjectImpl<ValueTuple<double, string, object>, ValueTuple>();
    }
    static void TupleConversionsObjectImpl<T, TTuple>()
    {
        TupleCodec<TTuple>.Register();
    ...
    {
        TupleRoundtripObjectImpl<ValueTuple<double, string, object>, ValueTuple>();
    }
    static void TupleRoundtripObjectImpl<T, TTuple>()
    {
        var tuple = Activator.CreateInstance(typeof(T), 42.0, "42", new object());
        using var pyTuple = TupleCodec<TTuple>.Instance.TryEncode(tuple);
        Assert.True(TupleCodec<TTuple>.Instance.TryDecode(pyTuple, out object restored));
        Assert.Equal(expected: tuple, actual: restored);
    }
    ...
    {
        TupleRoundtripGenericImpl<ValueTuple<int, string, object>, ValueTuple>();
    }

    static void TupleRoundtripGenericImpl<T, TTuple>()
    {
        var tuple = Activator.CreateInstance(typeof(T), 42, "42", new object());
        using var pyTuple = TupleCodec<TTuple>.Instance.TryEncode(tuple);
        Assert.True(TupleCodec<TTuple>.Instance.TryDecode(pyTuple, out T restored));
        Assert.Equal(expected: tuple, actual: restored);
    }
    ...
    public void ListDecoderTest()
    {
        ...
        Assert.True(codec.CanDecode(pyListType, typeof(IList<bool>)));
        Assert.True(codec.CanDecode(pyListType, typeof(IList<int>)));
        Assert.False(codec.CanDecode(pyListType, typeof(System.Collections.IEnumerable)));
        Assert.False(codec.CanDecode(pyListType, typeof(IEnumerable<int>)));
        Assert.False(codec.CanDecode(pyListType, typeof(ICollection<float>)));
        Assert.False(codec.CanDecode(pyListType, typeof(bool)));
        ...
        Assert.False(codec.CanDecode(pyListType, typeof(List<int>)));

        //convert to list of int
        IList<int> intList = null;
        Assert.Null(Record.Exception(() => { codec.TryDecode(pyList, out intList); }));
        (intList).Should().BeEquivalentTo(new List<object> { 1, 2, 3 });
        ...
        Assert.Null(Record.Exception(() => { codec.TryDecode(pyList, out stringList); }));
        ...
        Assert.False(codec.CanDecode(fooType, typeof(IList<int>)));
    }
    ...
    public void SequenceDecoderTest()
    {
        ...
        Assert.False(codec.CanDecode(listType, typeof(bool)));
        Assert.False(codec.CanDecode(listType, typeof(IList<int>)));
        Assert.False(codec.CanDecode(listType, typeof(System.Collections.IEnumerable)));
        Assert.False(codec.CanDecode(listType, typeof(IEnumerable<int>)));

        Assert.True(codec.CanDecode(listType, typeof(ICollection<float>)));
        Assert.True(codec.CanDecode(listType, typeof(ICollection<string>)));
        Assert.True(codec.CanDecode(listType, typeof(ICollection<int>)));

        //convert to collection of int
        ICollection<int> intCollection = null;
        Assert.Null(Record.Exception(() => { codec.TryDecode(pyList, out intCollection); }));
        (intCollection).Should().BeEquivalentTo(new List<object> { 1, 2, 3 });
        ...
        Assert.Null(Record.Exception(() => { codec.TryDecode(pyList, out stringCollection); }));
        Assert.Equal(3, stringCollection.Count());
        ...
        Assert.False(codec.CanDecode(fooType, typeof(ICollection<int>)));
        ...
        Assert.True(codec.CanDecode(pyTupleType, typeof(ICollection<float>)));
        Assert.True(codec.CanDecode(pyTupleType, typeof(ICollection<int>)));
        Assert.True(codec.CanDecode(pyTupleType, typeof(ICollection<string>)));

        //convert to collection of int
        ICollection<int> intCollection2 = null;
        Assert.Null(Record.Exception(() => { codec.TryDecode(pyTuple, out intCollection2); }));
        (intCollection2).Should().BeEquivalentTo(new List<object> { 1, 2, 3 });
        ...
        Assert.Null(Record.Exception(() => { codec.TryDecode(pyTuple, out stringCollection2); }));
        Assert.Equal(3, stringCollection2.Count);
        Assert.Throws<InvalidCastException>(() => {
            string[] array = new string[3];
    ...
    public void IterableDecoderTest()
    {
        ...
        Assert.False(codec.CanDecode(pyListType, typeof(IList<bool>)));
        Assert.True(codec.CanDecode(pyListType, typeof(System.Collections.IEnumerable)));
        Assert.True(codec.CanDecode(pyListType, typeof(IEnumerable<int>)));
        Assert.False(codec.CanDecode(pyListType, typeof(ICollection<float>)));
        Assert.False(codec.CanDecode(pyListType, typeof(bool)));

        //ensure a PyList can be converted to a plain IEnumerable
        System.Collections.IEnumerable plainEnumerable1 = null;
        Assert.Null(Record.Exception(() => { codec.TryDecode(pyList, out plainEnumerable1); }));
        (plainEnumerable1.Cast<PyInt>().Select(i => i.ToInt32())).Should().BeEquivalentTo(new List<object> { 1, 2, 3 });
        ...
        Assert.True(codec.CanDecode(pyListType, typeof(IEnumerable<int>)));
        Assert.True(codec.CanDecode(pyListType, typeof(IEnumerable<double>)));
        Assert.True(codec.CanDecode(pyListType, typeof(IEnumerable<string>)));

        IEnumerable<int> intEnumerable = null;
        Assert.Null(Record.Exception(() => { codec.TryDecode(pyList, out intEnumerable); }));
        (intEnumerable).Should().BeEquivalentTo(new List<object> { 1, 2, 3 });

        Runtime.CheckExceptionOccurred();

        IEnumerable<double> doubleEnumerable = null;
        Assert.Null(Record.Exception(() => { codec.TryDecode(pyList, out doubleEnumerable); }));
        (doubleEnumerable).Should().BeEquivalentTo(new List<object> { 1, 2, 3 });

        Runtime.CheckExceptionOccurred();

        IEnumerable<string> stringEnumerable = null;
        Assert.Null(Record.Exception(() => { codec.TryDecode(pyList, out stringEnumerable); }));
        ...
        Assert.Null(Record.Exception(() => { codec.TryDecode(pyList, out plainEnumerable2); }));
        (plainEnumerable2.Cast<PyInt>().Select(i => i.ToInt32())).Should().BeEquivalentTo(new List<object> { 1, 2, 3 });

        //can convert to any generic ienumerable.  If the type is not assignable from the python element
        //it will be an exception during TryDecode
        Assert.True(codec.CanDecode(fooType, typeof(IEnumerable<int>)));
        Assert.True(codec.CanDecode(fooType, typeof(IEnumerable<double>)));
        Assert.True(codec.CanDecode(fooType, typeof(IEnumerable<string>)));
...
import clr
import System
from CodeBrix.Python import PyObjectConversions
from CodeBrix.Python.Codecs import RawProxyEncoder
...
");
        ...
        Assert.Equal(TestExceptionMessage, message);
    }
    ...
    public void ExceptionDecoded()
    {
        ...
        Assert.Equal(TestExceptionMessage, error.Message);
    }
...
from CodeBrix.Python.Tests import DateTimeAcceptor, DateTimeDecoder

DateTimeDecoder.Setup()
");
        scope.Exec("DateTimeAcceptor.AcceptsDateTime(datetime(2021, 1, 22))");
    }
    ...
    public void As_Object_AffectedByDecoders()
    {
        ...
        Assert.Same(everythingElseToSelf, decoded);
    }
...
/// </summary>
public class ObjectToEncoderInstanceEncoder<T> : IPyObjectEncoder
{
...
public class DecoderReturningPredefinedValue<TTarget> : IPyObjectDecoder
{
...
/// <summary>Helper target for the DateTime codec test (called from Python).</summary>
public class DateTimeAcceptor
{
